...
`BLUEPRINT.md` §5.3 が固定しているのは
「**スマートパラメーターと `params` の競合はエラー**」であって、
**同一層の別名重複には触れていない**。round 1 が §5.3 を先例として引いたのは
スマートパラメーター側の話であり、**round 4/5 で決めた別名重複の規則は HISTORY
（H-0094）だけに載っている**。したがってそれを変えても**上位文書とは衝突しない** ——
Change Gate 案件ではあるが、BLUEPRINT の改訂は不要。
