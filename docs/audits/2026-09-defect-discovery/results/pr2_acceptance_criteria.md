...
### ⚠ 凍結は破れた —— 受け入れレビューのあと、管理者の指示で（2026-09-09）

**上の凍結は round 29（受け入れレビュー）が評価した head についてのものである。
その後、凍結は意図的に破られた。**

round 29 が返した 2 件を、**§6 は非 blocking と分類した**（#288 = B4、#287 = B3）。
**管理者はそのうえで、両方を本 PR 内で直すよう指示した。** 直っている。

| | 新しい head |
|---|---|
| production コード | **`77e02a8`**（`6b14b99` からの差分は **1 ファイル** = `lizyml/tuning/search_space.py`） |
| テスト | `9267c9a` |
| 契約 | 上記 + **H-0095 に追記した「探索空間の `choices` は型の同一性で判定する」決定** |
| 上位文書 | `BLUEPRINT.md` §5.3 / **§11.3** / §14.4 |

新しい凍結の検査:

```
git diff 6b14b99 <head> -- lizyml/    # lizyml/tuning/search_space.py の 1 件のみ
```

**これは §6 の完了経路の実行ではなく、§6 への例外である。**
rounds 28-29 の監視がこの点でこちらの記述を訂正した ——
§6 は「非 blocking な B3 は起票して終わり」「B4 は再レビューしない」と規定しており、
**B3 を PR 内で直す手順は §6 に書かれていない**。
「§6 の B1 が定める手順に入る」と述べたのは誤りである。
**管理者が承認した例外**として記録する。

そのため **round 30 を、選ばれた修正コミットに限定して**開く
（`22b11b3` / `77e02a8` / `9267c9a`）。**それを超えて広げない。**
監視が指定したとおり、**round 30 が選ばれた変更を超えて広がるか、さらなるラウンドを
自動的に生むなら、それは「ループが名前を変えて再開した」ことの証拠になる。**
...
| 6 | 決定 5（同一性マージ、4 継ぎ目）+ 決定 7（trial overlay = 5 つ目） | **`test_every_identity_overlay_seam_has_a_test`**（継ぎ目の母集団を `overlay_params` の呼び出し点から AST で導出し、各継ぎ目のテストに対応づける）／`test_the_estimator_never_sees_two_spellings_of_one_parameter`（`override`）／`test_the_tuning_result_seam_sends_one_spelling`（`best_model_params`）／`test_the_provider_fixed_seam_carries_one_spelling`（`fixed`）／`test_the_trial_overlay_seam_sends_one_spelling`（`model_p`） | round 29 は「2 継ぎ目しか通っていない」と指摘した。**#288 として本 PR で解決済み**（管理者の判断）。**4 件すべて RED 確認済み**。`fixed` の継ぎ目だけは**マージ後の dict** で主張する —— adapter が `metric` の全綴りを同一性で pop して canonical を書き戻すので、継ぎ目が両方残していても `lgb.train` は 1 綴りしか見ない。その理由は docstring に書いた |
...
| 項目 | クラス | バケット | 処分 |
|---|---|---|---|
| **[#287](https://github.com/nbx-liz/LizyML/issues/287)** —— 探索空間からサンプルされた値が入口の正規化を迂回し、study の内側で拒否される | DC7 | **B3、非 blocking** | **分類は非 blocking のまま。そのうえで管理者が本 PR 内で直すことを指示し、直した**（§6 への例外、上の §1 を参照）。**`isinstance` を型の同一性へ変え、すり抜けていた 2 型（`np.float64` / `np.str_`）を入口で拒否する。**Firing rate **0/54**（測定はスイート上のものであって全呼び出し元についてではない —— 監視の bound）。**#287 は OPEN のまま** —— 直したのはすり抜けであって、「4 surface が numpy を受理するのに探索空間は拒否する」という不整合は閉じていない。その問いのために開いておく |
| **[#288](https://github.com/nbx-liz/LizyML/issues/288)** —— 同一性マージの 4 継ぎ目のうち 2 つ（tuning 結果 / provider fixed）に、その層で別名を書くテストが無い | DC5 | **B4（テスト）** | **本 PR で解決した**（管理者の判断、2026-09-09）。繰り延べる利益がゼロ（テストのみ・設計判断なし・Change Gate 不要）で、この環境では未処理の繰り延べが post-closure audit の rework になる実績があるため。**継ぎ目の母集団を `overlay_params` の呼び出し点から AST で導出**し（`fixed` / `best_model_params` / `override` / `model_p`）、各継ぎ目に別名を書くテストを対応づけた。3 件は新規、1 件は既存。**4 件すべて RED 確認済み** —— 各継ぎ目を素の dict マージに戻すと対応するテストが落ち、6 つ目の継ぎ目を足すと導出テストが落ちる |
