...
- **The boosting round count is honoured under any of its names, and one parameter written twice with different values is refused** (H-0094, [#264](https://github.com/nbx-liz/LizyML/issues/264)). Only the literal `n_estimators` was turned into LightGBM's `num_boost_round`; `num_iterations`, `num_round` and the rest stayed in the parameter dict and reached `lgb.train` beside a different round count. They now all set the rounds. Separately, `fit(params={"objective": "binary", "application": "cross_entropy"})` — one parameter under two spellings with two values — now raises `CONFIG_INVALID` naming both, instead of letting the library decide which applies. The same values under two spellings are still accepted.
...
- **Two spellings of one parameter are refused instead of both reaching LightGBM, on every layer** (H-0094, [#264](https://github.com/nbx-liz/LizyML/issues/264)). The rule that one parameter written twice under two spellings with different values raises `CONFIG_INVALID` was applied to `fit(params=...)` only. A config carrying both `learning_rate: 0.001` and `eta: 0.5` — in `model.params` or in `calibration.params` — sent both to `lgb.train` and the library silently kept the canonical one. Both layers are now refused, naming both spellings and the layer they came from. The same value under two spellings is still accepted. Measured over every config this repository's suite constructs: **0 of 813** pre-existing configs with `model.params` and **0 of 22** with `calibration.params` are affected.

- **Equal array values written under two spellings are accepted again** (H-0094, [#264](https://github.com/nbx-liz/LizyML/issues/264)). `fit(params={"feature_contri": np.array([1, 2]), "feature_contrib": np.array([1.0, 2.0])})` names one parameter twice with the same value, and each array trains on its own — but the duplicate check compared array-valued parameters by their printed forms, so the differing dtypes read as two values and the call was refused. Values that convert to plain Python are now compared there; only values with no such conversion (a `DataFrame`, a custom object) fall back to printed forms.
...
- **A sequence and its comma-separated text form are one value** (H-0094, [#264](https://github.com/nbx-liz/LizyML/issues/264)). LightGBM writes every sequence parameter as `",".join(...)` and passes a string through unchanged, so `feature_contri: [1, 2]` and `feature_penalty: "1,2"` are the same value on the wire — each trained, and trained the identical booster, but writing both was refused with `CONFIG_INVALID`. They are now compared elementwise (not by the joined text, which is not canonical: `[1.0, 2.0]` and `[1, 2]` join differently and mean the same). Nested forms such as `interaction_constraints: [[0, 1], [2]]` versus `"[0,1],[2]"` are deliberately **not** covered — write those in one form.
...
- **Two search dimensions naming one parameter are refused instead of one being optimised over for nothing** (H-0094, [#264](https://github.com/nbx-liz/LizyML/issues/264)). A `category: model` space with both `learning_rate` and `eta` sampled both on every trial and put both in the parameter dict, so LightGBM kept the canonical one and **the other dimension had no effect on any trial** — Optuna ranked the trials on an axis that did nothing, and `best_model_params` recorded the dead value, which the `fit` afterwards then carried. Two dimensions spelling one LightGBM parameter now raise `CONFIG_INVALID` before the study starts, naming both. Unlike the parameter dicts there is no same-value exemption: two dimensions sample independently. Measured: **0 of 69** pre-existing `category: model` spaces this repository's suite constructs are affected. (Distinct from [#279](https://github.com/nbx-liz/LizyML/issues/279), a dimension colliding with a smart parameter, which is still open.)

- **A list, a tuple and an array of the same numbers are one value** (H-0094, [#264](https://github.com/nbx-liz/LizyML/issues/264)). The same duplicate check treated the container as part of the value, so `{"feature_contri": np.array([1., 2.]), "feature_penalty": (1., 2.)}` — one LightGBM parameter, written twice with the same numbers — was refused with `CONFIG_INVALID` and nothing trained, on both `model.params` and `fit(params=...)`. LightGBM trains the byte-identical booster from any of the three, so there was nothing to disambiguate. Sequences are now compared by their contents; `str` and `bytes` are not treated as sequences.
...
- **A parameter value that refuses conversion no longer makes `fit()` raise** (H-0094, [#264](https://github.com/nbx-liz/LizyML/issues/264)). Naming one parameter twice — once as a sequence or number, once as LightGBM's comma-separated text form — compares the two. The comparison converted each element inside a guard that named only `TypeError` and `ValueError`, and printed it outside any guard at all, so a value whose `__float__` or `__str__` raises anything else came straight out of `fit()`. A `str` subclass whose `split` raises did the same thing from the other side, and the repair for that then refused a string-like object that trains fine on its own. The guarantee is now stated against LightGBM's own parameter serialiser, which is the thing that decides what counts as one value: **the check does not raise on any value LightGBM accepts, and admits any pair LightGBM would treat as one value.** A value LightGBM itself refuses is outside that guarantee, because it cannot complete training under any spelling. The comparison also uses the same formatter LightGBM uses for each position -- it writes a scalar parameter one way and a sequence element another, and using one for both both refused a pair that writes identical bytes and, worse, **reported two genuinely different values as the same**. The formatted result is reduced to its plain characters as well, so a value that formats to a string subclass cannot have that subclass decide the comparison.
