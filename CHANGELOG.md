...
- **A parameter value that refuses conversion no longer makes `fit()` raise** (H-0094, [#264](https://github.com/nbx-liz/LizyML/issues/264)). Naming one parameter twice — once as a sequence or number, once as LightGBM's comma-separated text form — compares the two. The comparison converted each element inside a guard that named only `TypeError` and `ValueError`, and printed it outside any guard at all, so a value whose `__float__` or `__str__` raises anything else came straight out of `fit()`. A `str` subclass whose `split` raises did the same thing from the other side. The module documents that it does not raise; it now does not, and the equivalent pair is still admitted rather than merely survived.
