...
- **Two spellings of one parameter are refused instead of both reaching LightGBM, on every layer** (H-0094, [#264](https://github.com/nbx-liz/LizyML/issues/264)). The rule that one parameter written twice under two spellings with different values raises `CONFIG_INVALID` was applied to `fit(params=...)` only. A config carrying both `learning_rate: 0.001` and `eta: 0.5` — in `model.params` or in `calibration.params` — sent both to `lgb.train` and the library silently kept the canonical one. Both layers are now refused, naming both spellings and the layer they came from. **Since H-0096 the values are not read at all** — see the H-0096 entry below. Measured over every config this repository's suite constructs: **0 of 813** pre-existing configs with `model.params` and **0 of 22** with `calibration.params` are affected.

- ~~**Equal array values written under two spellings are accepted again**~~ — **superseded by H-0096 before release; nothing shipped with this behaviour.** The entry described a repair to how array-valued parameters were compared. H-0096 removed the comparison entirely, so `fit(params={"feature_contri": np.array([1, 2]), "feature_contrib": np.array([1.0, 2.0])})` is refused — for naming one parameter under two spellings, not for a judgement about the arrays. Each array written **once** still trains, unchanged.
...
- ~~**A sequence and its comma-separated text form are one value**~~ — **superseded by H-0096 before release; nothing shipped with this behaviour.** LightGBM does write every sequence parameter as `",".join(...)`, so `feature_contri: [1, 2]` and `feature_penalty: "1,2"` reach it as the same bytes, and each still trains on its own. Writing **both** is refused, as any duplicate spelling now is. The elementwise comparison this entry announced is gone with the rest of the comparison.
