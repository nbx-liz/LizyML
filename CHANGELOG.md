...
- **Two spellings of one parameter in `model.params` are refused instead of both reaching LightGBM** (H-0094, [#264](https://github.com/nbx-liz/LizyML/issues/264)). The rule that one parameter written twice under two spellings with different values raises `CONFIG_INVALID` was applied to `fit(params=...)` only, so a config carrying both `learning_rate: 0.001` and `eta: 0.5` sent both to `lgb.train` and the library silently kept the canonical one. It is now refused, naming both spellings and `model.params`. The same value under two spellings is still accepted. Measured over every config this repository's suite constructs: **0 of 811** pre-existing configs are affected.
