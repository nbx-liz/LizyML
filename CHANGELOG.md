...
- **A parameter value that refuses conversion no longer makes `fit()` raise** (H-0094, [#264](https://github.com/nbx-liz/LizyML/issues/264)). Naming one parameter twice — once as a sequence or number, once as LightGBM's comma-separated text form — compares the two. The comparison converted each element inside a guard that named only `TypeError` and `ValueError`, and printed it outside any guard at all, so a value whose `__float__` or `__str__` raises anything else came straight out of `fit()`. A `str` subclass whose `split` raises did the same thing from the other side, and the repair for that then refused a string-like object that trains fine on its own. The guarantee is now stated against LightGBM's own parameter serialiser, which is the thing that decides what counts as one value: **the check does not raise on any value LightGBM accepts, and admits any pair LightGBM would treat as one value.** A value LightGBM itself refuses is outside that guarantee, because it cannot complete training under any spelling.
