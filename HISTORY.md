...
2. `BaseMetric` に `needs_normalized_proba` 属性を追加する（案 C）
   - 不採用。メトリクスごとの判定は過剰。multiclass の確率メトリクスは全て正規化が必要。

### Acceptance Criteria

- `multiclassova` の非正規化出力で `roc_auc_score` がエラーなく動作する。
- softmax 出力は `assert_allclose` で実質不変。
- OVA sigmoid 出力が行正規化され、行合計が 1.0 になる。
...
- Date: `2026-03-14`
- Result: `accepted`
- Notes: Evaluator 層での行正規化（案 B）を採用。`_normalize_multiclass_proba()` を `evaluator.py` に追加し、`_pred_for_metric()` の `needs_proba` 分岐に multiclass 2D チェックを追加する。
