...
## Phase 26: Calibration CV と Splitter 境界の整合

**HISTORY:** H-0044  
**SKILL:** skills/splitters-and-splitplan/SKILL.md, skills/calibration/SKILL.md, skills/testing/SKILL.md, skills/spec-update/SKILL.md  
**依存:** Phase 25

### 背景

BLUEPRINT §10.1 では splitter を outer CV / early stopping / calibration で共通利用する契約だが、現行の calibration cross-fit は `KFold` 固定で `split.method` を継承していない。これにより、group/time/purge/embargo 境界が calibration 段階で崩れる不一致がある。

### 26-A: calibration split 生成責務の分離

1. `lizyml/core/_model_factories.py` に calibration 用 splitter 構築ヘルパーを追加する。
   - `split.method` と split パラメーター（`gap` / `purge_gap` / `embargo` / `train_size_max` / `test_size_max`）を継承する。
   - fold 数のみ `calibration.n_splits` で上書きする。
2. `lizyml/core/model.py` の calibration パスで、上記 splitter から calibration split indices を作成する。
3. `fit_result.splits.calibration` に実使用 split indices を保存する（既存契約を維持）。

### 26-B: cross_fit_calibrate の IF 整理

1. `lizyml/calibration/cross_fit.py` を「split 生成」から「与えられた split で cross-fit 実行」へ責務縮小する。
2. 校正器学習入力は引き続き `s_oof` と `y` のみを使用し、`X` 非依存契約を維持する。
3. `c_final` は全データ学習・推論専用、評価は `calibrated_oof` のみという契約を維持する。

### 26-C: split.method 別の契約テスト追加

1. `tests/test_calibration/` に split.method 別テストを追加する。
   - `kfold` / `stratified_kfold`: `len(splits.calibration) == calibration.n_splits`
   - `group_kfold`: train/valid 間で group overlap がない
   - `time_series`: train_idx が valid_idx より過去のみ
   - `purged_time_series`: `purge_gap + embargo` 境界が満たされる
   - `group_time_series`: group/time 境界が満たされる
2. `split.n_splits != calibration.n_splits` のケースで outer と calibration が独立に動作する回帰テストを追加する。
3. `tests/test_e2e/test_leakage_all.py` の「cross-fit OOF != c_final」テストが維持されることを確認する。

### 26-D: 例外・診断性の補強

1. calibration 分割不能（`n_splits` 過大、group 不足など）の失敗を明示エラーとして統一する。
2. エラー context に `split_method`, `calibration_n_splits`, `n_samples`（可能なら group 数）を含める。

### 26-E: ドキュメント整合

1. `BLUEPRINT.md` の §10 / §12 と実装を一致させる（H-0044 の受け入れ後に Status 更新）。
2. Config Reference / Notebook の calibration 記述に「`split.method` 継承 + fold 数独立」を反映する。

### DoD

- 26-A: calibration split が `split.method` 継承で生成され、`fit_result.splits.calibration` に保存される。
- 26-B: `cross_fit_calibrate` が split 生成責務を持たず、cross-fit 実行責務に限定される。
- 26-C: split.method 別契約テスト（group/time/purge/embargo）が追加され全件 pass する。
- 26-D: 分割不能ケースで明示エラーと context が返る。
- 26-E: BLUEPRINT/HISTORY/PLAN の記述が相互に矛盾しない。

---

## Phase 27: Evaluation 表示契約の是正（fold_n=OOF）

**HISTORY:** H-0045  
**SKILL:** skills/evaluation-contracts/SKILL.md, skills/metrics/SKILL.md, skills/testing/SKILL.md, skills/spec-update/SKILL.md  
**依存:** Phase 26

### 背景

現行 `evaluate_table()` の `fold_0..fold_N-1` は IF（train_idx）値を表示しており、fold ばらつきの汎化監視として解釈しづらい。fold 列は OOF（valid_idx）値を表示する契約に変更する。

### 27-A: Evaluator 契約拡張（oof_per_fold）

1. `lizyml/evaluation/evaluator.py` に `raw.oof_per_fold` を追加する。
   - 各 outer fold の `valid_idx` 上で metric を計算する。
   - `raw.if_per_fold` / `raw.if_mean` は互換維持する。
2. 評価構造の docstring と型契約を更新する。

### 27-B: evaluate_table の fold 列意味変更

1. `lizyml/evaluation/table_formatter.py` の `fold_0..fold_N-1` を `oof_per_fold` 参照に変更する。
2. `if_mean` 列は互換のため維持する（参考値）。
3. `cal_oof` 列の既存挙動は維持する。

### 27-C: 契約テスト更新

1. `tests/test_evaluation/test_evaluation.py`
   - `oof_per_fold` 構造と valid_idx ベース算出を検証する。
2. `tests/test_evaluation/test_table_formatter.py`
   - `fold_n` が IF ではなく OOF-per-fold を表示することを検証する。
3. `tests/test_core/test_contracts.py`
   - metrics 階層のゴールデンに `oof_per_fold` を追加する。

### 27-D: 類似箇所監査（仕様確認）

1. IF/OOS 混在の可視化・テーブル API を棚卸しする。
   - `roc_curve_plot()`（IS/OOS）
   - `confusion_matrix()`（is/oos）
   - `residuals_plot()`（IS/OOS）
2. 「診断目的で IF を残す API」と「汎化監視で OOF を優先する API」を分類し、BLUEPRINT に明文化する。
3. 必要な追加仕様変更は別 H-entry として分離する（本 Phase では fold_n 問題を優先解消）。

### DoD

- 27-A: `evaluate()["raw"]["oof_per_fold"]` が追加され、各 fold valid_idx 指標と一致する。
